    ...
    <section className="bg-white py-24 px-4 md:px-12 lg:px-16 overflow-hidden">
      ...
      <div className="md:hidden text-center mb-8">
        <h2 className="[font-family:'Quicksand',Helvetica] text-2xl sm:text-3xl font-bold uppercase tracking-wide">
          ...
          <span className="text-[#37A65E]">Work</span>
        </h2>
      </div>
        ...
        <div className="md:hidden flex flex-col gap-4">
          {steps.map((step) => (
            <div key={step.id} className="flex items-start gap-3">
              ...
              <p className="[font-family:'Quicksand',Helvetica] text-xs sm:text-[13px] font-semibold text-[#37A65E] leading-snug pt-0.5">
                {step.text}
              </p>
            </div>
          ))}
            ...
            {row1.map((step, index) => (
              <div key={step.id} className="flex-1 flex flex-col items-center md:items-start group relative w-full md:w-auto">
                ...
                <div className="h-[65px] flex items-end justify-start mb-6 px-2 w-full md:max-w-[180px]">
                  <p className="[font-family:'Quicksand',Helvetica] text-xs sm:text-[13px] font-semibold text-[#37A65E] text-left leading-snug">
                    {step.text}
                  </p>
                </div>
                ...
                <motion.div
                  ...
                  className="ml-2 sm:ml-4 w-11 h-11 rounded-full bg-[#37A65E] flex items-center justify-center text-white font-bold text-lg shadow-lg relative z-10 hover:scale-110 transition-transform"
                >
                  {step.id}
                </motion.div>
          ...
          <div className="flex flex-col md:flex-row-reverse items-start justify-between relative gap-12 md:gap-4 mt-4 md:-mt-8">
            {row2.map((step, index) => (
              <div key={step.id} className="flex-1 flex flex-col items-center md:items-start group relative w-full md:w-auto">
                ...
                {step.id < 10 && (
                  <div className="md:hidden mt-4">
                     <svg className="h-8 w-4 text-[#37A65E]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                     </svg>
                  </div>
                )}
