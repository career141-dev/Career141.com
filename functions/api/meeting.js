  ...
  try {
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        to: [
          {
            email: env.CONTACT_RECIPIENT_EMAIL || "info@career141.com",
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!success) {
      return json({ error: "Failed to send request. Please try again later." }, 500);
    }

    return json({ message: "Request sent successfully" }, 200);
  } catch (error) {
